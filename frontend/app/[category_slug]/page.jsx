...
export default async function Store({ params, searchParams }) {
  ...
  // ---------- Query string banao — page + filters sab ek sath ----------
  const queryString = new URLSearchParams();
  
  queryString.set("page", currentPage);
  if (query?.min_price) queryString.set("min_price", query.min_price);
  if (query?.max_price) queryString.set("max_price", query.max_price);


  // ---------- Products fetch (category ke sath) ----------
  try {
    const response = await api.get(
      `/store/${category_slug}/?${queryString.toString()}`
    );
  ...
  // ---------- Pagination link banate waqt filters preserve karo ----------
  const buildPageHref = (pageNum) => {
    const p = new URLSearchParams(queryString);
    p.set("page", pageNum);
    return `/${category_slug}?${p.toString()}`;
  };
